...
test("corpus: exact rung + code-set for every case", () => {
  for (const c of corpus.cases) {
    ...
    for (const code of c.expect.notCheckedIncludes || []) {
      assert.ok(
        v.notChecked.some((n) => n.code === code),
        `${c.name}: expected notChecked ${code}`,
      );
    }
  }
});
...
test('kernel never certifies "verified"', () => {
  for (const c of corpus.cases)
    assert.equal(smell(c.evidence as Evidence, DEFAULTS).verifiedCheckmark, false);
});
...
test("claims: clause-scoped negation does not over-cancel", () => {
  ...
  assert.ok(
    c.some((x) => x.kind === "implemented"),
    "implemented survives a negator in a different clause",
  );
...
test("claims: abbreviation/decimal guard keeps the trailing claim", () => {
  ...
  assert.ok(
    s1.some((s) => /^Implemented/.test(s)),
    'the "Implemented" sentence survives the e.g. boundary',
  );
