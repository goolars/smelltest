...
export function reconcile(claims: Claim[], ev: Evidence, cfg: Config): Reconciliation {
  ...
  if (implFix && (!ev.diff || ev.diff.available === false)) {
    notChecked.push({
      ...
      reason: "no git/diff context to verify the completion claim",
    });
  }
  ...
  if (ev.diff && ev.diff.available !== false) {
    ...
    if ((ev.diff.hunks || []).some((h) => h.combined)) {
      notChecked.push({
        code: "combined-diff-unsupported",
        reason: "merge/combined (--cc) diff — substance not graded for that file",
      });
    }

    if (implFix && !hasRemoved && sub.substantiveAddedLines === 0) {
      const ungradable =
        sub.classifiedFiles.length === 0 && (ev.diff.hunks || []).some((h) => h.binary || h.combined);
      if (sub.classifiedFiles.length === 0 && sub.unsupportedFiles.length > 0) {
        ...
        notChecked.push({
          code: "done.no_substance",
          reason: `changed only unsupported-language file(s): ${sub.unsupportedFiles.slice(0, 4).join(", ")}`,
        });
      } else if (ungradable) {
        ...
        notChecked.push({
          code: "done.no_substance",
          reason: "changed only binary / merge file(s) — substance not gradable",
        });
      } else {
        ...
        findings.push({
          ...
          signal: {
            substantiveAddedLines: 0,
            filesTouched: ev.diff.filesTouched.length,
            classified: sub.classifiedFiles,
          },
        });
      }
    }
      ...
      if (tamper.tampered) {
        ...
        try {
          ...
          if (looksTest.length && !touched.some((f) => testRe.test(f)) && !tamper.testFiles.length) {
            notChecked.push({
              code: "test-selector-miss",
              reason: `test-looking files changed but the test selector matched none: ${looksTest.slice(0, 3).join(", ")}`,
            });
          }
        } catch {
          /* validated config => unreachable, but never crash the gate */
        }
      }
    }
  }

  if (ev.scope?.filesEdited?.length) {
    if (ev.scope.filesRead == null) {
      ...
      if (blind.length) {
        findings.push({
          ...
          severity: "advisory",
          message: `Edited ${blind.length} file(s) never read this session: ${blind.slice(0, 4).join(", ")}.`,
          signal: { blind },
        });
      }
    }
