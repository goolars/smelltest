...
const ABBREV = /\b(?:e\.g|i\.e|etc|vs|cf|al|fig|no|vol|eq|approx|dr|mr|mrs|ms|st|jr|sr|inc|ltd|co|u\.s|a\.m|p\.m)\.$/i;
...
function findLexeme(clause: string, lexicon: Record<string, string[]>): { kind: ClaimKind; lexeme: string; idx: number } | null {
  ...
  for (const kind of Object.keys(lexicon)) {
    for (const phrase of lexicon[kind]) {
      ...
      const m = re.exec(lo);
      if (m && (best === null || m.index < best.idx)) best = { kind: kind as ClaimKind, lexeme: phrase, idx: m.index };
    }
  }
  return best;
}
...
export function extractClaims(text: string, cfg: Config): Claim[] {
  ...
  for (const sentence of segmentSentences(text)) {
    for (const clause of splitClauses(sentence)) {
      ...
      if (!hit) continue;
      claims.push({ kind: hit.kind, lexeme: hit.lexeme, negated: hasNegator(clause, cfg.negators), sentence });
    }
  }
  return claims;
}
