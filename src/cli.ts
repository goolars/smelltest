...
import { smell, renderVerdict } from './kernel.ts';
...
function normalizeEvidence(p: any): Evidence {
  ...
  const diff = okDiff
    ? { available: p.diff.available !== false, isEmpty: !!p.diff.isEmpty, filesTouched: Array.isArray(p.diff.filesTouched) ? p.diff.filesTouched : [], hunks: p.diff.hunks }
    : { available: false, isEmpty: true, filesTouched: [], hunks: [] };
  const scope = p && typeof p.scope === 'object' && p.scope
    ? { filesRead: Array.isArray(p.scope.filesRead) ? p.scope.filesRead : null, filesEdited: Array.isArray(p.scope.filesEdited) ? p.scope.filesEdited : [] }
...
function main(): void {
  ...
  if (sub === 'arm') {
    ...
    console.log(`smelltest enforcement ARMED. Bounded: max ${cfg.bounds.maxRevisions} revisions, session-independent ceiling, fail-open. Disarm: smelltest disarm`);
    return;
  }
  if (sub === 'disarm') { try { fs.rmSync(armedFile, { force: true }); } catch { /* noop */ } console.log('smelltest enforcement DISARMED (advisory only).'); return; }
  if (sub === 'status') { console.log(`smelltest: enforcement ${fs.existsSync(armedFile) ? 'ARMED' : 'disarmed (advisory)'}; maxRevisions=${cfg.bounds.maxRevisions}`); return; }

  let ev: Evidence;
  if (has('--stdin')) {
    ev = normalizeEvidence(JSON.parse(fs.readFileSync(0, 'utf8')));
  } else if (has('--transcript') || has('--latest')) {
    ev = buildEvidence({ transcriptPath: has('--latest') ? findLatestTranscript() : get('--transcript'), root });
  } else {
