...
const IMPORT_ONLY = /^(import\b|export\s+\{|export\s+\*|from\s+['"]|#include\b|using\s+\w|const\s+\w+\s*=\s*require\()/;
const PUNCT_ONLY = /^[\s{}()[\];,.:<>+|&]*$/;
...
export interface TamperResult { tampered: boolean; signals: string[]; testFiles: string[]; }
...
export function detectTestTamper(diff: DiffInfo, cfg: Config): TamperResult {
  ...
  for (const h of diff.hunks || []) {
    ...
    if (removedAsserts > addedAsserts) signals.push(`net -${removedAsserts - addedAsserts} assertion(s) in ${h.file}`);
