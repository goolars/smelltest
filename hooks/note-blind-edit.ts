...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (ev.scope.filesRead == null) {
      ledger.append(root, cfg, { event: 'notChecked', sessionId: input.session_id, code: 'scope.blind_edit', reason: 'no read-set' });
      return;
    }
    if (!new Set(ev.scope.filesRead.map(norm)).has(norm(edited))) {
      ledger.append(root, cfg, { event: 'advisory', sessionId: input.session_id, code: 'scope.blind_edit', file: edited });
      console.log(JSON.stringify({ systemMessage: `smelltest: edited a file not read this session — ${path.basename(edited)}.` }));
    }
  } catch { /* fail-open */ }
}
void main();
